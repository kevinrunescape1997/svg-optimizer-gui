...
Adds:
- File counter in the progress overlay label (File x/y • y queued)
- Percent tracker for current job (stage-based since backends don't expose granular progress)
- Preserve folder structure toggle (like the other GUIs)
- Proper drag & drop wiring using provided DND_FILES
- Global mousewheel scrolling for list and canvas (consistent UX)
- Respect recursive setting when scanning folders
- Custom name all / Custom stem all toggles and fields
...
try:
    from svg_exporter import convert_svg, SUPPORTED_FORMATS  # type: ignore
except Exception as e:
    raise RuntimeError(f"Failed to import svg_exporter. Make sure it is in PYTHONPATH. Error: {e}")
...
def is_svg_file(p: Path) -> bool:
    return p.suffix.lower() in SVG_SUFFIXES


def find_svgs_in_folder(folder: Path, recursive: bool) -> list[Path]:
...
class App:
    ...
    def _build_ui(self):
        ...
        self.listbox = tk.Listbox(
            lb_wrap, selectmode=tk.EXTENDED, bg=DARK_SURFACE, fg=DARK_TEXT,
            highlightthickness=0, bd=0, selectbackground=PRIMARY_BG_ACTIVE, selectforeground="#ffffff",
        )
        ...
        self.drop_label = tk.Label(
            drop_wrap,
            text=("Drag and drop SVG/SVGZ files or folders here.\n\n\n\n\n\n"
        ...
        self.btn_add_files = ttk.Button(add_wrap, text="Add SVG/SVGZ Files", command=self._guard(self.add_files))
        ...
        ttk.Label(dpi_row, text="DPI (PNG/TIFF):").pack(side="left")
        ttk.Entry(dpi_row, textvariable=self.dpi, width=8).pack(side="left", padx=(8, 0))

        preserve_row = ttk.Frame(fmt_wrap)
        preserve_row.pack(fill="x", pady=(TOGGLE_ROW_SPACING, 0))
        ttk.Checkbutton(preserve_row, text="Preserve folder structure", variable=self.preserve_tree, style="OnOff.TCheckbutton").pack(side="left")
    ...
    def _handle_input_paths(self, paths: list[Path]):
        ...
        for folder in folders:
            svgs = find_svgs_in_folder(folder, recursive=self.recursive.get())
    ...
    def _set_controls_enabled(self, enabled: bool):
        widgets = [
            self.header_open_btn, self.run_btn, self.btn_add_files, self.btn_add_folder,
            self.btn_clear, self.btn_remove, self.btn_choose_out, self.btn_up,
        ]
    ...
    def add_files(self):
        def do_pick():
            picked = system_pick_files("Select SVG/SVGZ files", patterns=["*.svg", "*.svgz"], parent_winid=self._get_winid())
            if picked is None:
                picked = list(
                    filedialog.askopenfilenames(
                        title="Select SVG/SVGZ files",
                        filetypes=[("SVG files", "*.svg *.svgz"), ("All files", "*.*")],
                        parent=self.root,
                    )
                )
            elif picked == []:
    ...
    def add_folder(self):
        ...
        if not svgs:
            messagebox.showinfo("No SVGs found", "No .svg or .svgz files found in the selected folder.", parent=self.root)
            return
        self._add_paths(svgs)
    ...
    def _wire_preview_updates(self):
        for var in (self.rename_all, self.rename_base, self.use_custom_stem, self.custom_stem, self.output_format, self.preserve_tree):
            try:
    ...
    def _update_naming_preview(self) -> None:
        if not hasattr(self, "naming_preview") or self.naming_preview is None:
            return
        lines: list[str] = []
        total_files = len(self.files)
        fmt = self.output_format.get().strip().lower()
        suffix = self._suffix_for(fmt)
    ...
    def run(self):
        if not self.files:
            messagebox.showwarning("Nothing to do", "Add at least one SVG/SVGZ file.", parent=self.root)
            return
    ...
    def _run_worker(self, out_dir: Path):
        ...
        try:
            dpi_val = int(self.dpi.get()) if self.dpi.get().strip() else None
        except Exception:
            dpi_val = None
        ...
        for idx1, inp in enumerate(self.files, start=1):
            ...
            base_pct_for_prev = ((idx1 - 1) / max(1, total_files)) * 100.0
            per_file_span = 100.0 / max(1, total_files)

            stem = self._compute_output_stem(inp, idx1 - 1)
            out_path = (Path(self.output_dir.get()).expanduser() / (stem + self._suffix_for(fmt)))
            if preserve_tree and common_root is not None:
                try:
                    rel_parent = inp.parent.relative_to(common_root)
                    out_path = Path(self.output_dir.get()).expanduser() / rel_parent / out_path.name
                except Exception:
                    pass
            ...
            try:
                ok, msg = convert_svg(inp, fmt, out_path, dpi=dpi_val)
            except Exception as e:
                ok, msg = False, str(e)
            ...
            if ok:
                results.append(JobResult(inp, out_path, True, msg))
            else:
    ...
    def _suffix_for(self, fmt: str) -> str:
        return { "pdf": ".pdf", "png": ".png", "eps": ".eps", "tiff": ".tiff" }.get(fmt, ".out")
    ...
    def _finish(self, results: list[JobResult], out_dir: Path):
        ...
        log_path = out_dir / "svg_export_log.txt"
        lines = []
        for r in results:
    ...
    def _show_completion_dialog(self, results: list[JobResult], out_dir: Path, log_path: Path):
        ...
        txt.insert("1.0", "\n".join([f"{'✓' if r.ok else '✗'} {r.input_path.name} -> {r.output_path.name} | {r.message}" for r in results]))
